...
if not HGGetFileData then
  function HGGetFileData(file)
...
local function createNumbers(node, str, padding, rightAlignment, colorStr, addition)
  ...
  for index = 1, #str do
    ...
    local path = (oripath or "UI/alpha/HVGA/digits/" .. colorStr) .. "/" .. c .. (suffix or ".png")
...
local function setKeyBack(node, func)
  ...
  HGLog("registerScriptKeypadHandler setKeyBack tools")
  ...
  if func then
    ...
    keypadLayer:registerScriptKeypadHandler(function(event)
      if event == "backClicked" then
      	HGLog("registerScriptKeypadHandler")
        ed.popScene()
...
local getNodeSize = function(node, bWithMessageRect)
  ...
  --HGLog("getAnchorPoint node is: " .. tolua.type(node) .." dump ".. tostring(node));
  local anchor = node:getAnchorPoint()
  local posX, posY
...
local openAppStore = function()
  if HGFLAGIOS then
   -- local addr = "itms-apps://itunes.apple.com/cn/app/xxxxxx/id793077082?mt=8"
    local addr = "itms-apps://itunes.apple.com/app/heroes-charge/id900313219?mt=8"
    if HGFLAG_ALPHAVERSION then
        addr = "http://hkm.Aegwynn.com/ios/hero.html"
    end
    HGOpenURL(addr)
  elseif HGFLAGANDROID then
   local addr = "https://play.google.com/store/apps/details?id=com.Aegwynn.hero"
   if HGFLAG_ALPHAVERSION then
        addr = "http://hkm.Aegwynn.com/android/hero.html"
   end
   HGOpenURL(addr)
  end
end
...
local function formatText(text)
  ...
  if EDLanguage == "chinese" then
    if HGFLAGANDROID and HGSDKType == 104 then
