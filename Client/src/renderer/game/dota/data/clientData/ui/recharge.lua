...
local function onChargeRsp(self)
  local function handler(data)
    ...
    if 3 == HGSDKType then
      HGBuyDiamond(1, goodInfo.title, goodInfo.cost, data._serial_id, string.format("%s:%d:%s", game_server_ip, data._charge_id, ed.getUserid()))
    end
    
    if 117 == HGSDKType then
      local pInfo = {
        id = ed.getUserid(),
      ...
      HGAndroidPurchase(ed.getJson(pInfo))
    end
  end
  return handler
end
...
local function doRecharge(self, info)
  ...
  if HGFLAGIOS then
    if 3 == HGSDKType then
      local msg = ed.upmsg.charge()
      msg._platid = 3
      msg._chargeid = info.id
      ed.send(msg, "charge")
    elseif 2 == HGSDKType then
      HGBuyDiamond(1, info.title, info.cost, info.id, string.format("%s:%s:%s", game_server_ip, info.id, ed.getUserid()))
    else
      HGBuyDiamond(1, info.title, info.cost, info.id, game_server_id,ed.getDeviceId())
    end
  end
  local itemid = tostring(info.id)
  local un = ed.player:getName()
  un = string.gsub(un, "\"", "[qm]")
  if HGFLAGANDROID then
--    if HGSDKType == 102 then
--      local pInfo = {
--        cpInfo = {item = itemid, serverid = game_server_id},
--        ip = game_server_id,
--        balance = ed.player._rmb,
--        cost = info.cost,
...
--      }
--      HGAndroidPurchase(ed.getJson(pInfo))
--    elseif HGSDKType == 104 then
--      local pInfo = {
--        cpInfo = {item = itemid, serverid = game_server_id},
--        accessToken = ed.getAccessToken(),
--        uin = ed.getDeviceId(),
--        cost = info.cost,
--        rate = info.get / info.cost,
--        url = ed.getRechargeURL(),
--        itemName = info.title,
--        item = itemid,
--        gameName = LSTR("RECHARGE.TURRET_LEGEND"),
--        userName = un,
...
--      }
--      HGAndroidPurchase(ed.getJson(pInfo))
--    elseif HGSDKType == 106 then
--      local pInfo = {
--        cpInfo = {item = itemid, serverid = game_server_id},
--        radio = info.get / info.cost,
--        cost = info.cost,
--        name = LSTR("RECHARGE.DIAMOND")
--      }
--      HGAndroidPurchase(ed.getJson(pInfo))
--    elseif HGSDKType == 107 then
--      local pInfo = {
--        cpInfo = {item = itemid, serverid = game_server_id},
--        id = ed.getUserid(),
--        item = itemid,
...
--        userid = ed.getDeviceId(),
        ...
        -- for php post data same as ios post data
        uin = ed.getDeviceId(),
