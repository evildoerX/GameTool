...
local function doLogin(self)
  ...
  login._old_deviceid = game_server_id
...
--        end
--      else
        EDEnterGameNotifiy((ed.tutorial.isFinishTutorial and 1) or 0)
--  end
    end, EDDebug)
  end)
...
local function create(sessionId)
  ...
  local latestVersion = T(LSTR("SERVERLOGIN.VERSION_")) ..CCUserDefault:sharedUserDefault():getStringForKey("current-version")
...
local function checkLatestVersion(self)
  local function handler()
    ...
    local currentVersion = CCUserDefault:sharedUserDefault():getStringForKey("current-version")
    HGLog("Latest Version: " .. latestVersion)
    ...
    if auto_update_switch then
      ...
      if not self.autoUpdateEof then
        ...
        elseif LoadResources:getCode() == 4  then
          ...
          HGLog("--autoUpdateEof " .. tostring(progress));
        ...
        elseif LoadResources:getCode() == 3 then
          ...
          --  HGLog("--refreshBar " .. tostring(progress));
          ...
          if LoadResources:isEnd() and not self.autoUpdateEof then
            ...
            if  HGFLAGIOS or HGFLAGANDROID then
