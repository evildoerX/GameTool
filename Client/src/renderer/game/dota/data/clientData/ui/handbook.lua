...
local tagNormalColor = ccc3(255, 255, 255)
local tagSelectColor = ccc3(182, 65, 21)
local clickTime
local tagTextIndex = {
  ALL = 1,
  STR = 2,
...
local function doSelectTag(self, id)
  ...
  ed.setLabelColor(self.tagLabel[pid], ccc3(255, 255, 255))
  ...
  ed.setLabelColor(self.tagLabel[id], ccc3(182, 65, 21))
      ...
      if event == "began" then
        for i = 1, 12 do
          if ed.containsPoint(self.tagButton[i], x, y) then
            ...
            label:disableShadow()
            label:setZOrder(9)
            ed.setLabelColor(label, ccc3(182, 65, 21))
          end
        end
      elseif event == "ended" then
        if id then
...
local function setPageTitle(self, listName, page)
  if (self.currentListName or "") ~= listName then
    do
      local pageTitle = ed.createttf(tagText["title" .. tagTextIndex[listName]], 18, "Sling.ttf")
      ed.setLabelColor(pageTitle, ccc3(192, 65, 21))
  ...
  if (self.currentListName or "") == listName then
  elseif (self.pageid or 0) ~= page then
