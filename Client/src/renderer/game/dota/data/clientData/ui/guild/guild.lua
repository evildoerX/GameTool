...
local function setGuildMerberNum()
  ed.setString(guildPanel.mainLayer.members, T("%d/%d", memberNum, EDTables.guildConfig.maxGuildMember))
...
local function showGuildInfo(data, worshipData, dropData)
  ...
  ed.setString(mainLayer.id, data._summary._id)
...
function guild.guildInstance()
  local ds = ed.playerlimit.getAreaUnlockPrompt("Raid")
  if ds then
    ed.showToast(ds)
    return
  end
...
local getMemberDes = function(type, data)
  ...
  if type == "time" then
    ...
    if d == nowD and passSecond < 86400 or data._last_login == 0 then
      timeDes = T(LSTR("guild.1.10.010"), h, min)
    else
      local days = math.ceil(passSecond / 86400)
      timeDes = T(LSTR("guild.1.10.011"), days)
    end
    ...
    for i, v in ipairs(data._items) do
      if v._item_info then
        ...
        for _, item in ipairs(v._item_info) do
          ...
          if endTime > ed.getServerTime() then
