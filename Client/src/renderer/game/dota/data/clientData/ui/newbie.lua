...
local function dispatch(msg)
  ...
  if msg.login_reply then
    if msg.login_reply.error_code == 0 then
      ...
      HGLog("ed.ui.newbie.uin:" .. ed.ui.newbie.uin)
      HGLog("ed.config.tokendata:" .. ed.config.tokendata)
    ...
    HGLog("login errcode:" .. tostring(msg.login_reply.error_code))
    if currentUi ~= nil then
      currentUi:setVisible(true)
      local action = CCFadeIn:create(fadinlength)
      currentUi:runAction(action)
  ...
  if msg.create_user_reply then
    ...
    HGLog("create_user_reply errcode:" .. tostring(msg.create_user_reply.error_code))
    if currentUi ~= nil then
      currentUi:setVisible(true)
      local action = CCFadeIn:create(fadinlength)
      currentUi:runAction(action)
  ...
  if msg.logout_reply and msg.logout_reply.error_code == 0 then
    ...
    HGLog("logout errcode:" .. tostring(msg.logout_reply.error_code))
    ed.showAlertDialog({
  ...
  if msg.reset_passwd_reply then
    ...
    elseif msg.reset_passwd_reply.error_code == 2 then
      ...
      HGLog("reset_passwd_reply errcode:" .. tostring(msg.reset_passwd_reply.error_code))
    end
  end
  if msg.update_passwd_reply then
    ...
    elseif msg.update_passwd_reply.error_code == 7 then
      ...
      HGLog("update_passwd_reply errcode:" .. tostring(msg.update_passwd_reply.error_code))
    end
  end
  if msg.deviceinfo_reply and msg.deviceinfo_reply.error_code == 0 then
    HGLog("deviceinfo_reply errcode:" .. tostring(msg.deviceinfo_reply.error_code))
  end
end
...
local function connect(block)
  ...
  if connect_succ then
    ...
    if r1 then
      ...
      HGLog(string.format("ERROR | connection:send failed | %s", err))
    end
    close()
  elseif block then
...
local resetTokenFile = function()
  ...
  if os.rename(path, path) then
    os.remove(path)
    HGLog("TokenRemoved:" .. path)
  else
    HGLog("TokenNotExist:" .. path)
  end
end
