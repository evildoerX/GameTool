...
function changeFacebookConnectState(self, state)
  ...
  if  state == 1 then
    ...
    HGLog("changeFacebookConnectState connected")
    -- connect success, change button to disconnect
    ...
    HGLog("changeFacebookConnectState disconnected")
  ...
  local ui_info = {
    ...
    },
    {
      ...
      base = {
        ...
        text = T(LSTR("CONFIGURE.SYSTEM.SUPPORT")),
        fontinfo = "ui_normal_button",
        size = 20,
        parent = "support_button"
      },
...
local function createWindow(self)
  ...
  if self:showGoogleConnect() then
    ...
    self:changeFacebookConnectState(HGIsFacebookConnected())
  end



  if HGSDKType == 104 then
  ...
  local function handler(event, x, y)
    ...
    elseif event == "ended" then
      if isPress then
        ...
        if ed.containsPoint(facebook_connect, x, y) then
          if HGIsFacebookConnected()==1 then
            HGDisconnectFacebook()
...
local doClickWebButton = function(self)
  local addr = "http://hero.Aegwynn.com"
  if HGFLAGANDROID then
    HGAndroidExtra("openURL", addr)
  elseif HGFLAGIOS then
    HGOpenURL(addr)
  end
end
...
--  if EDFLAGWP8 then
--    HGLog("ed.ui.newbie.doLogout")
--    ed.ui.newbie.doLogout()
--  else
    HGLogoff(HGSDKType)
--  end
end
    ...
    elseif event == "ended" then
      if isPress then
        ...
        if ed.containsPoint(button, x, y) then
          ...
          HGAndroidExtra("realNameRegister", ed.getJson(info))
        end
      end
      isPress = nil
    end
    ...
    elseif event == "ended" then
      if isPress then
        ...
        if ed.containsPoint(button, x, y) then
          ...
          HGAndroidExtra("checkAntiAddiction", ed.getJson(info))
        end
      end
      isPress = nil
    end
...
local function showLogoff(self)
  ...
  --  return false
  --  if HGFLAGANDROID and not ed.isElementInTable(HGSDKType, {
  --    102,
  --    104,
  --    109,
  --    110,
