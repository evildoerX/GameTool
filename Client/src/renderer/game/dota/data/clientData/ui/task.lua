...
local function createFastButton(self, task, param)
  ...
  local fast_handler = {
    ...
    MonthlyCardPeriod = function()
      local recharge = ed.ui.recharge.create()
      self.mainLayer:addChild(recharge.mainLayer, 200)
    end,
  ...
  if handler and isShow() then
    ...
    button:setContentSize(CCSizeMake(90, 40))
    press:setContentSize(CCSizeMake(90, 40))
    press:setAnchorPoint(ccp(0, 0))
    press:setPosition(ccp(0, 0))
    press:setVisible(false)
    button:addChild(press)
    label = ed.createttf(labelString or T(LSTR("TASK.HEAD_TO")), 18, "Sling.ttf")
    label:setPosition(ccp(45, 19))
