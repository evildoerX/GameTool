...
local function createPrompt(self, param)
  ...
  if ps then
    ...
    ed.setLabelColor(psLabel, ccc3(238, 204, 119))
  ...
  function ed.netreply.syncTime()
    xpcall(function()
      if not tolua.isnull(self.mainLayer) then
        ed.setString(self.ui.title, T(LSTR("DAILYLOGIN._D_MONTHLY_ATTENDANCE_AWARDS"), os.date("%B", os.time({year=2014, month=getDate().m, day=1, hour=1,min=0,sec=0}))))
        self:createList()
      end
    end, EDDebug)
  end
