...
local btRegisterHandler = function(self, param)
  if not param then
    HGLog("U must set the param of the handler! --handler")
  end
...
local btGetClickHandler = function(self, param)
  if not param then
    HGLog("U must set the param of the button handler! --click")
    return
  end
...
local function btGetOutClickHandler(self, param)
  if not param then
    HGLog("U must set the param of the button handler! --out click")
    return
  end
...
local function btGetButtonClickHandler(self, param)
  ...
  if not param then
    HGLog("U must set the param of the button handler! --button click")
    return
  end
...
local function btGetCircleClickHandler(self, param)
  if not param then
    HGLog("U must set the param of the button handler! --circle click")
    return
  end
...
local btRegisterMainTouchHandler = function(self, param)
  ...
  if key then
    ...
    elseif list[key] then
      HGLog("The base Touch handler --<<" .. key .. ">>-- has already set!")
      return
    end
    list[key] = handler
  else
