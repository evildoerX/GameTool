...
function sendmailwithattachment(filename, label)
  ...
  from = "<hgbugreport@163.com>"
  to = "<hgbugreport@163.com>"
  rcpt = {
    "<hgbugreport@163.com>"
  }
  mesgt = {}
  if filename == nil then
    mesgt = {
      headers = {
        to,
        subject = "HgGame-LuaCrashLog " .. label
      },
...
function senddumpfile(logstr)
  ...
  data:close()
  ziplog(dumpfile)
  sendmailwithattachment(dumpfile .. ".zip", "cr ios " .. ed.getDeviceId() .. " " .. ed.getUserid() .. " " .. ed.player:getName() .. " " .. CCUserDefault:sharedUserDefault():getStringForKey("current-version"))
  os.remove(dumpfile)
