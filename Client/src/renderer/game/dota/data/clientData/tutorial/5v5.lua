...
local function start5v5()
  ...
  local Axe = ed.engine:findMonster("SG")
  Axe.config.hp_mod = 4.5
  Axe:rebuild()
  Axe.hp = Axe.attribs.HP
  Axe.mp = 1000
  skill = Axe.skills.SG_ult
  skill.info["Plus Ratio"] = 10
  local Med = ed.engine:findMonster("Luna")
  Med.config.hp_mod = 3
  Med.config.dps_mod = 3
  Med:rebuild()
  Med.hp = Med.attribs.HP
  Med.mp = 50
  Med.skills.Luna_ult.info["Plus Ratio"] = 5
  ...
  local Bone = ed.engine:findMonster("DR")
  Bone.config.hp_mod = 3
  Bone:rebuild()
  ...
  if HGFLAGANDROID then
    ed.scene.google_play_btn:setVisible(true)
  end
end
