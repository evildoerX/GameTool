...
local function refreshAndroid()
  ...
  HGAndroidExtra("setAlarmData", ed.getJson(nInfo))
end
class.refreshAndroid = refreshAndroid
local function refreshIOS()
  for i = 1, #data do
    local nd = data[i]
    local date = getNotifyDate(i)
    local key = nd.iKey
    ...
    local tag = nd.iTag
    if date > 0 and getSwitch(i) then
      HGCancelNotification(key)
      HGAddNotification(key, text, tag, date)
    else
      HGCancelNotification(key)
    end
  end
end
class.refreshIOS = refreshIOS
...
local function init()
  if HGFLAGANDROID then
    ...
    HGAndroidExtra("clearAlarm", ed.getJson(ids))
  end
  if HGFLAGIOS then
  end
  refresh()
