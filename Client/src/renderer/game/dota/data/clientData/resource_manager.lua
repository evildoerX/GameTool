...
if HGSetAniScaleFactor then
  HGSetAniScaleFactor(ed.cha_scale)
end
...
local function createSprite(resource)
  ...
  if frame then
    ...
    HGLog("Failed to creating sprite from " .. (resource or "nil"))
...
local function createFcaNode(resource)
  local isUI = string.match(resource, "^eff_UI")
  if isUI then
    HGSetAniScaleFactor(ed.cha_ui_scale)
  else
    HGSetAniScaleFactor(ed.cha_scale)
  end
  print(resource)
  local node = HGAminationEffect:create(resource)
  HGSetAniScaleFactor(ed.cha_scale)
  return node
end
...
local function createFcaActor(resource)
  ...
  local node = HGAnimation:create(resource, 1)
  HGSetAniScaleFactor(ed.cha_scale)
  return node
end
