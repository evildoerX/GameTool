...
local cryptkey = "0"
local user_id = 0
local device_id = ed.config.uin or HGGetDeviceID()
...
local function pack_size(code, ttype)
  ...
  --if not EDFLAGWIN32 and not EDFLAGWP8 then
   -- device = string.format("%d-%s", HGSDKType, device_id)
  --end
  ...
  device_len = string.len(device_id)
  ...
  return int_to_bytes(pkglength, nil, "signed") .. int_to_bytes(reservedLength, nil, "signed") .. int_to_bytes(dataSize, nil, "signed").. string.char(mode) .. third .. fourth .. device_id.. code
end
...
local function connect(block)
  --HGLog("function connect call \n" .. debug.traceback())
  local err
  if not connection then 
    connection, err = socket.tcp()
    if not connection then
      HGLog(string.format("ERROR | Connect failed when creating socket | %s", err))
      return false
    end
  end
  ...
  if connect_succ then
    ...
    if r1 then
      HGLog("<<< Message Sent  @ " .. ed.getYMDTime())
    else
      HGLog(string.format("ERROR | connection:send failed | %s", r2))
      close()
    end
    --close()
  elseif block then
    HGLog(string.format("ERROR | Blocked Connect failed | %s", err))
    return false
  else
    HGLog("    connect start")
    return true
...
local function doSend(block, ttype)
  ...
  -- HGLog("============Send Msg Key: "..cryptkey)
  HGLog("============doSend Msg Type: "..ttype)
  -- HGLog(print_bytes(code))
  -- HGLog("encrypt data============")
  -- HGLog(print_bytes(up_code))
...
local function send(obj, ttype, block)
  ...
  sdkInfo._session_key = CCUserDefault:sharedUserDefault():getStringForKey("current-version")
  sdkInfo._plat_id = 0
  if HGFLAGIOS then 
    sdkInfo._plat_id = 1
  elseif HGFLAGANDROID then
    sdkInfo._plat_id = 2
  end
...
local function recv_remain(proto_head_str)
  --[[for i = 1, 12 do
    HGLog("    recv_remain " .. string.byte(proto_head_str, i))
  end
  --]]
...
local function recv()
  ...
  if (commandId == 1) then
    HGLog("    proxy timeout close : " .. lastHeartBeatTime)
    close()
    return
  elseif (commandId == 46) then
    HGLog("    onPongProxy : " .. lastHeartBeatTime)
    recv_remain(proto_head_str)
    return
  elseif (commandId ~= 47) then
    HGLog("    other message : " .. lastHeartBeatTime)
    recv_remain(proto_head_str)
    return
  end
  ...
  if buffer then
    HGLog(">>> Data Received @ " .. ed.getYMDTime())
    loadEnd()
    local plaincode = mycrypto.decrypt(buffer, cryptkey)

    logString = plaincode
    
    -- HGLog("Msg Data Received Begin============")
    -- HGLog(print_bytes(buffer))
    -- HGLog("=====")
    -- HGLog(print_bytes(plaincode))
    -- HGLog("Msg Data Received End============")
    ...
    if msg then
      ...
      HGLog("ERROR | Decode message failed | " .. err)
      EDDebug()
    end
  end
end
...
local function dispatch(msg)
  ...
  if msg._reset then
    HGLog("Client data reset by server!")
  ...
  if msg._notify_msg then
    HGLog("===============_notify_msg");
    HGLog(var_dump(msg._notify_msg));
